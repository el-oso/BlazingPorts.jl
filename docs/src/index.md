...
| **simd-json** | JSON.jl ≥1.6 (+ stage-1 SIMD POC, fork) | **full parse 0.66× simd-json — gap stands.** A stage-1 string-scan POC wins *long* strings (crossover ~48 B → 2.8× at 512 B) but is ~0.85× on short ones | ⚠ **Gap / POC** (+ fixed StrictMode F32) |
...
| **hex** (enc+dec) | `ByteOps.hex_encode!`/`hex_decode!` (ours) | **beats faster-hex: encode 1.01× (AVX2), decode 1.26×**; 7×/43× `bytes2hex`/`hex2bytes`; byte-exact + validating | ✅ **PORTED** (SIMD pshufb) |
